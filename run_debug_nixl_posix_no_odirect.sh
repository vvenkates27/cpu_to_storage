#!/usr/bin/env bash
# run_debug_nixl_posix_no_odirect.sh — quick single-run debug test for nixl POSIX backend, no O_DIRECT
#
# Equivalent to:
...
#     --threads 16 --test-name debug_nixl_posix_no_odirect
#
# Background (survives logout) — pick one:
#
#   nohup (simplest, no reattach):
#     nohup bash run_debug_nixl_posix_no_odirect.sh > bench_debug_nixl_posix_no_odirect.log 2>&1 &
#     echo $!                            # save the PID
#     tail -f bench_debug_nixl_posix_no_odirect.log
#
#   screen (reattachable):
#     screen -S debug_nixl_posix_no_odirect
#     bash run_debug_nixl_posix_no_odirect.sh 2>&1 | tee bench_debug_nixl_posix_no_odirect.log
#     Ctrl-A D  (detach)  /  screen -r debug_nixl_posix_no_odirect  (reattach)
#
#   tmux (reattachable):
#     tmux new -s debug_nixl_posix_no_odirect
#     bash run_debug_nixl_posix_no_odirect.sh 2>&1 | tee bench_debug_nixl_posix_no_odirect.log
#     Ctrl-B D  (detach)  /  tmux attach -t debug_nixl_posix_no_odirect  (reattach)
...
NIXL_BACKEND="POSIX"
THREADS=16
TEST_NAME="debug_nixl_posix_no_odirect"
# ─────────────────────────────────────────────────────────────────────────────
...
# ── Run start banner ──────────────────────────────────────────────────────────
log "======================================================================"
log "  DEBUG nixl POSIX no O_DIRECT  —  started at $(date)"
log "======================================================================"
log "  Mode:          $MODE"
...
# ── debug_nixl_posix_no_odirect ───────────────────────────────────────────────
run_benchmark "$TEST_NAME" \
    "nixl POSIX | no O_DIRECT | threads: $THREADS | block-sizes: $BLOCK_SIZES MB" \
    $COMMON --backend $BACKEND --nixl-backend $NIXL_BACKEND --no-o-direct \
    --threads $THREADS \
    --test-name $TEST_NAME
